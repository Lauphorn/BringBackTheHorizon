...
namespace SCPE
{
    public class SCPE_GUI : Editor
    {
        ...
        public static void DisplaySetupWarning<T>(bool state)
        {
#if URP && PPS
            if (UniversalRenderPipeline.asset.postProcessingFeatureSet == PostProcessingFeatureSet.Integrated)
            {
            ...
            {
                GUILayout.FlexibleSpace();
                if (GUILayout.Button(new GUIContent("Open", EditorGUIUtility.IconContent("d_tab_next").image), GUILayout.Width(60)))
                {
                    ...
                    Selection.activeObject = forwardRenderer;
                    //Application.OpenURL("http://staggart.xyz/unity/sc-post-effects/scpe-docs/?section=setup-using-urp-2019-3");
                }
                GUILayout.Space(8);
            }
            GUILayout.Space(11);
