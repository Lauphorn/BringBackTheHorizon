Shader "Hidden/SC Post Effects/Color Split"
{
	...
	float4 FragSingle(Varyings2 i) : SV_Target
	{
		...
		float4 original = ScreenColor(i.texcoord[0].xy);
	...
	float4 FragSingleBoxFiltered(Varyings2 i): SV_Target
	{
		...
		float red = BoxFilter4(RT_PARAM(_MainTex, sampler_LinearClamp), i.texcoord[0].zw, _MainTex_TexelSize.xy * (_Offset * 200), 1.0).r;
		float4 original = BoxFilter4(RT_PARAM(_MainTex, sampler_LinearClamp), i.texcoord[0].xy, _MainTex_TexelSize.xy * (_Offset * 200), 1.0);
		float blue = BoxFilter4(RT_PARAM(_MainTex, sampler_LinearClamp), i.texcoord[1].xy, _MainTex_TexelSize.xy * (_Offset * 200), 1.0).b;
	...
	float4 FragDoubleBoxFiltered(Varyings3 i) : SV_Target
	{
		...
		float4 redX = BoxFilter4(TEXTURE2D_PARAM(_MainTex, sampler_MainTex), i.texcoord[0].zw, _MainTex_TexelSize.xy * (_Offset * 200), 1.0);
		float4 redY = BoxFilter4(TEXTURE2D_PARAM(_MainTex, sampler_MainTex), i.texcoord[1].zw, _MainTex_TexelSize.xy * (_Offset * 200), 1.0);

		float4 original = BoxFilter4(TEXTURE2D_PARAM(_MainTex, sampler_MainTex), i.texcoord[0].xy, _MainTex_TexelSize.xy * (_Offset * 200), 1.0);

		float4 blueX = BoxFilter4(TEXTURE2D_PARAM(_MainTex, sampler_MainTex), i.texcoord[1].xy, _MainTex_TexelSize.xy * (_Offset * 200), 1.0);
		float4 blueY = BoxFilter4(TEXTURE2D_PARAM(_MainTex, sampler_MainTex), i.texcoord[2].xy, _MainTex_TexelSize.xy * (_Offset * 200), 1.0);
